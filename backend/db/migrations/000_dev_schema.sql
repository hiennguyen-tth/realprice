-- ============================================================
-- RealPrice Dev Schema (no PostGIS — for local development)
-- Uses plain lat/lng instead of GEOGRAPHY types
-- ============================================================
...
CREATE TYPE legal_status_enum AS ENUM (
  'so_do', 'so_hong', 'giay_to_hop_le', 'chua_co_giay_to', 'dang_lam_so'
);
CREATE TYPE listing_status_enum AS ENUM (
  'pending_review', 'active', 'sold', 'expired', 'hidden'
);
...
-- ============================================================
-- LANDS (lat/lng only — no GEOGRAPHY)
-- ============================================================
CREATE TABLE lands (
  id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_lands_lat_lng   ON lands(lat, lng);
...
-- ============================================================
-- AREA PRICE INDEX (no GEOGRAPHY boundary)
-- ============================================================
...
CREATE TRIGGER trg_lands_updated_at      BEFORE UPDATE ON lands           FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_listings_updated_at   BEFORE UPDATE ON listings        FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_comparisons_updated_at BEFORE UPDATE ON price_comparisons FOR EACH ROW EXECUTE FUNCTION set_updated_at();
